...
vector<Stop> readStopsFile(const string &filename, map<string,int> &stopsIndex) {
    ...
    if (!stopsFile.is_open()) {
        ...
        return stops; // Return an empty vector
    }
    ...
    while (!stopsFile.eof()){
        ...
        stopsIndex.insert(make_pair(stop.code, i));
...
void readLineStops (vector<Line> &allLines) {
    for (auto &l : allLines){
        stringstream ss0("data/line_" + l.code + "_0.csv");
        ...
        stringstream ss1("data/line_" + l.code + "_1.csv");
...
int main() {
    ...
    vector<Stop> stops = readStopsFile("data/stops.csv", stopsIndex);
    vector<Line> allLines = readLinesFile("data/lines.csv");
